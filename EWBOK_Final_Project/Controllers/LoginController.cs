...
namespace EWBOK_Final_Project.Controllers
{
    public class LoginController : Controller
    {
        ...
        public ActionResult ConfirmCode(User user)
        {
            ...
            new LogDao().SetLog("ConfirmCode", null, ((User)Session[Constants.USER_INFO]).ID);
            return View(user);
        }
        [HttpPost]
        public ActionResult ConfirmCode(string code, long id)
        {
            ...
            if (code == user.CodeConfirm)
            {
                ...
                new LogDao().SetLog("ConfirmCode", "Thành công", ((User)Session[Constants.USER_INFO]).ID);
                return RedirectToAction("Index");
            }
            else
            {
                TempData[Constants.ALERT_CONFIRMPASSWORD] = "Sai mã xác nhận";
                new LogDao().SetLog("ConfirmCode", "Sai mã xác nhận", ((User)Session[Constants.USER_INFO]).ID);
        ...
        public ActionResult ForgotPassword(string email, string username)
        {
            ...
            new LogDao().SetLog("Change Password", "Thất bại", user.ID);
